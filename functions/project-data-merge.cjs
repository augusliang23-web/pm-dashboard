...
function canonicalValue(value) {
  ...
  if (value && typeof value.toMillis === 'function') return { $type: 'timestamp', value: value.toMillis() };
...
function identifyRows(rows, kind, idField = 'id') {
  ...
  return rows.map((source, index) => ({ ...source, [idField]: ensureStableEditorRowId(source, kind, index) }));
}

function withProjectEditorRowIds(project = {}) {
  const identified = { ...project };
  const rowFields = [
    ['riskActions', 'risk-action', 'id'], ['riskPairs', 'risk-pair', 'id'], ['teamMembers', 'team', 'id'], ['milestones', 'milestone', 'planId'],
  ];
  for (const [field, kind, idField] of rowFields) {
    if (Array.isArray(project[field])) identified[field] = identifyRows(project[field], kind, idField);
  }
  if (Array.isArray(project.ganttWorkstreams)) {
    identified.ganttWorkstreams = project.ganttWorkstreams.map((source, index) => ({
      ...source, id: String(source?.id || '').trim() || `workstream-${index + 1}`,
    }));
  }
  ...
  if (Array.isArray(project.quarterlyMilestones)) {
    identified.quarterlyMilestones = project.quarterlyMilestones.map((source, index) => {
      const item = Array.isArray(source) ? { quarter: source[0], goal: source[1], window: source[2], status: source[3] } : source;
      return { ...item, id: ensureStableEditorRowId(item, 'qms', index) };
    });
  }
  if (isPlainObject(project.budget)) {
    identified.budget = { ...project.budget };
    if (Array.isArray(project.budget.monthlyPlans)) identified.budget.monthlyPlans = identifyRows(project.budget.monthlyPlans, 'budget-plan');
    if (Array.isArray(project.budget.actuals)) identified.budget.actuals = identifyRows(project.budget.actuals, 'budget-actual');
  }
  return identified;
}
...
function mergePreservingUnknown(liveValue, draftValue) {
  if (Array.isArray(draftValue)) {
    if (!Array.isArray(liveValue)) return draftValue.map(item => mergePreservingUnknown(undefined, item));
    const usedLiveIndexes = new Set();
    return draftValue.map((draftItem, draftIndex) => {
      const identity = arrayEntryIdentity(draftItem);
      let liveIndex = -1;
      if (identity) {
        liveIndex = liveValue.findIndex((liveItem, index) => !usedLiveIndexes.has(index) && arrayEntryIdentity(liveItem) === identity);
      } else if (!isPlainObject(draftItem) && draftIndex < liveValue.length && !usedLiveIndexes.has(draftIndex)) {
        liveIndex = draftIndex;
      }
      if (liveIndex >= 0) usedLiveIndexes.add(liveIndex);
      return mergePreservingUnknown(liveIndex >= 0 ? liveValue[liveIndex] : undefined, draftItem);
    });
  }
  if (isPlainObject(draftValue)) {
    const liveObject = isPlainObject(liveValue) ? liveValue : {};
    const merged = { ...liveObject };
    for (const [key, value] of Object.entries(draftValue)) merged[key] = mergePreservingUnknown(liveObject[key], value);
    return merged;
  }
  return draftValue;
}

...
function renderRiskActions(model) {
  ...
  const table = dataTable({ headings: ['Project', 'Risk / Blocker', 'Escalation', 'Owner', 'Mitigation Actions', 'Checkpoint'], rows, className: 'risk-action-table' })
  ...
  return flowItem({
    ...
    pageSection: 'overview-management',
    splittable: true,
    body: `<section class="overview-unit risk-actions" data-section-unit="risk-actions"><div class="overview-unit-head"><div><div class="report-kicker">Decision-oriented risk list</div><h2 class="pdf-continuation-label">Risk &amp; Mitigation Actions</h2></div><span class="overview-note">Primary risk and action pair</span></div>${table}</section>`
  });
}

...
const readRules = () =>
  readFile(new URL("../firestore.uat.rules", import.meta.url), "utf8").catch(
    () => "",
  );
...
const readDashboard = () =>
  dashboardSourceAsync('uat').catch(() => "");
...
test("firebase config maps the Firestore rules source", async () => {
  const config = JSON.parse(
    await readFile(new URL("../firebase.uat.json", import.meta.url), "utf8"),
  );

  assert.equal(config.firestore?.rules, "firestore.uat.rules");
});

test("firebase config includes Executive history query indexes", async () => {
  const config = JSON.parse(await readFile(new URL("../firebase.uat.json", import.meta.url), "utf8"));
  const indexes = JSON.parse(await readFile(new URL("../firestore.uat.indexes.json", import.meta.url), "utf8"));

  assert.equal(config.firestore?.indexes, "firestore.uat.indexes.json");
